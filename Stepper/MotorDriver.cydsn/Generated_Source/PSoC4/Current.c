...
#define SetP4PinDriveMode(shift, mode)  \
    do { \
        Current_PC =   (Current_PC & \
                                (uint32)(~(uint32)(Current_DRIVE_MODE_IND_MASK << (Current_DRIVE_MODE_BITS * (shift))))) | \
                                (uint32)((uint32)(mode) << (Current_DRIVE_MODE_BITS * (shift))); \
    } while (0)
...
{
    uint8 drVal = (uint8)(Current_DR & (uint8)(~Current_MASK));
    drVal = (drVal | ((uint8)(value << Current_SHIFT) & Current_MASK));
    Current_DR = (uint32)drVal;
}
...
{
	SetP4PinDriveMode(Current__0__SHIFT, mode);
}
...
{
    return (uint8)((Current_PS & Current_MASK) >> Current_SHIFT);
}
...
{
    return (uint8)((Current_DR & Current_MASK) >> Current_SHIFT);
}
...
/* If Interrupts Are Enabled for this Pins component */ 
    ...
    *******************************************************************************/
    ...
    {
		uint8 maskedStatus = (uint8)(Current_INTSTAT & Current_MASK);
		Current_INTSTAT = maskedStatus;
        return maskedStatus >> Current_SHIFT;
    }
